...
interface CargoHoras {
  ...
  tipo:
    | "comunes"
    | "investigacion"
    | "mision_especial_primaria"
    | "mision_especial_secundaria"
    | "mision_especial"
    | "regimen_27"
    | "regimen_5";
...
export default function ProfesionalForm({ vm }: Props) {
  ...
  useEffect(() => {
    if (currentProfesional) {
      setForm({
        ...
        fechaVencimientoPsicofisico:
          currentProfesional.fechaVencimientoPsicofisico ?? "",
        fechaVencimientoMatricula:
          currentProfesional.fechaVencimientoMatricula ?? "",
        ...
        direccion: {
          ...
          departamentoId: currentProfesional.direccion?.departamento?.id
            ? String(currentProfesional.direccion.departamento.id)
            : "",
        },
        tieneReduccion: currentProfesional.tieneReduccion ?? false,
        motivoReduccion: currentProfesional.motivoReduccion ?? "",
        horasReduccion: currentProfesional.horasReduccion ?? 0,
      });
    } else {
      setForm(emptyForm);
    }
  ...
  const handleEquipoRemove = (id: number) => {
    setForm((prev: any) => ({
      ...prev,
      equiposIds: prev.equiposIds.filter((eid: number) => eid !== id),
    }));
  };

  const handleCargoHorasChange = (
    index: number,
    field: keyof CargoHoras,
    value: string | number,
  ) => {
  ...
  const removeCargoHoras = (index: number) => {
    setForm((prev: any) => ({
      ...prev,
      cargosHoras: prev.cargosHoras.filter((_: any, i: number) => i !== index),
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        ...(form.fechaVencimientoPsicofisico && {
          fechaVencimientoPsicofisico: form.fechaVencimientoPsicofisico,
        }),
        ...(form.fechaVencimientoMatricula && {
          fechaVencimientoMatricula: form.fechaVencimientoMatricula,
        }),
        ...
        ...(form.direccion?.calle && {
          direccion: {
            ...
            departamentoId: form.direccion.departamentoId
              ? parseInt(form.direccion.departamentoId)
              : undefined,
          },
        }),
      };
  ...
  const equiposDisponibles = (equipos ?? []).filter(
    (eq: any) => !(form.equiposIds ?? []).includes(eq.id),
  );
  ...
  return (
    ...
    >
      ...
      <DialogContent className="w-[95vw] sm:max-w-[1000px] max-h-[90vh] overflow-y-auto p-4">
        <DialogHeader>
          <DialogTitle>
            {currentProfesional ? "Editar Profesional" : "Agregar Profesional"}
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-2">
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="min-w-0">
                <Label htmlFor="fechaVencimientoMatricula">
                  Vto. Matrícula
                </Label>
                <Input
                  ...
                  type="date"
                  className="w-full min-w-0"
                  value={form.fechaVencimientoMatricula || ""}
                  onChange={(e) =>
                    setForm((p: any) => ({
                      ...p,
                      fechaVencimientoMatricula: e.target.value,
                    }))
                  }
                />
              </div>
              <div className="min-w-0">
                <Label htmlFor="fechaVencimientoPsicofisico">
                  Vto. Psicofísico
                </Label>
                <Input
                  ...
                  type="date"
                  className="w-full min-w-0"
                  value={form.fechaVencimientoPsicofisico}
                  onChange={handleChange}
                />
              </div>
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="min-w-0">
                ...
                <Select
                  onValueChange={(value) =>
                    handleDireccionChange("departamentoId", value)
                  }
                  value={form.direccion.departamentoId}
                >
                  <SelectTrigger
                    id="direccion.departamentoId"
                    className="w-full min-w-0"
                  >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-3">
                {(form.cargosHoras ?? []).map(
                  (cargo: CargoHoras, index: number) => (
                    <div
                      key={index}
                      className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-end border p-3 rounded-lg"
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <Select
                          value={cargo.tipo}
                          onValueChange={(v) =>
                            handleCargoHorasChange(index, "tipo", v as any)
                          }
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="mision_especial_primaria">
                              Misión Especial Primaria
                            </SelectItem>
                            <SelectItem value="mision_especial_secundaria">
                              Misión Especial Secundaria
                            </SelectItem>
                            <SelectItem value="mision_especial">
                              Misión Especial
                            </SelectItem>
                            <SelectItem value="regimen_27">
                              Régimen 27
                            </SelectItem>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
                {/* Checkbox tieneReduccion */}
                <div className="flex items-center gap-2">
                  <input
                    id="tieneReduccion"
                    type="checkbox"
                    checked={form.tieneReduccion}
                    onChange={(e) =>
                      setForm((prev: any) => ({
                        ...prev,
                        tieneReduccion: e.target.checked,
                        // Limpiar los campos opcionales si se desmarca
                        ...(!e.target.checked && {
                          motivoReduccion: "",
                          horasReduccion: 0,
                        }),
                    ...
                    className="h-4 w-4"
                  />
                  <Label htmlFor="tieneReduccion">Tiene Reducción</Label>
                </div>

                {/* Motivo — solo visible si tieneReduccion */}
                {form.tieneReduccion && (
                  <div className="min-w-0">
                    <Label htmlFor="motivoReduccion">Motivo de Reducción</Label>
                    <Input
                      id="motivoReduccion"
                      name="motivoReduccion"
                      className="w-full min-w-0"
                      value={form.motivoReduccion}
                      onChange={handleChange}
                    />
                  </div>
                )}

                {/* Horas — solo visible si tieneReduccion */}
                {form.tieneReduccion && (
                  <div className="min-w-0">
                    <Label htmlFor="horasReduccion">Horas de Reducción</Label>
                    <Input
                      id="horasReduccion"
                      name="horasReduccion"
                      type="text"
            ...
            <div>
              ...
              >
                <SelectTrigger id="equiposIds" className="w-full min-w-0">
