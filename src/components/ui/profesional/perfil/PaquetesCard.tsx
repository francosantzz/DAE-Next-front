...
// Importa tu AlertDialog (ajusta rutas si es necesario)
import {
  AlertDialog,
  AlertDialogContent,
  AlertDialogHeader,
...
export default function PaquetesCard({
  ...
  // Estado para el confirm único
  const [openConfirm, setOpenConfirm] = useState(false)
  const [selected, setSelected] = useState<PaqueteHorasPerfil | null>(null)
  const [isDeleting, setIsDeleting] = useState(false)

  const openDeleteDialog = (pk: PaqueteHorasPerfil) => {
    setSelected(pk)
    setOpenConfirm(true)
  }
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      await onEliminar(Number(selected.id))
  ...
  return (
    <>
                  ...
                  <div key={pk.id}
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex space-x-2">
                        ...
                        {/* Aquí abrimos el dialog en vez de eliminar inmediatamente */}
                        <PermissionButton
      ...
      {/* AlertDialog único, controlado por estado */}
      <AlertDialog open={openConfirm} onOpenChange={(o) => { if (!o) { setOpenConfirm(false); setSelected(null) } }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              ¿Eliminar a {selected ? (selected.tipo ? `${selected.tipo}` : 'este paquete') : 'este paquete'}?
            </AlertDialogTitle>
            <AlertDialogDescription>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => { setOpenConfirm(false); setSelected(null) }}>Cancelar</AlertDialogCancel>
            <AlertDialogAction
